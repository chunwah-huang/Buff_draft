...
void Find_couple_red::run_Main(Mat & srcImg){
    ...
    image_Processing(srcImg, result_img);
    if(is_run){
...
void Find_couple_red::image_Processing(Mat & src, Mat & binImg){
    vector<Mat> bgr;
    split(src, bgr);
    if(color_ == 0){
        subtract(bgr[2], bgr[1], binImg);
    }
    else{
        subtract(bgr[0], bgr[2], binImg);
    }
    threshold(binImg, binImg, 25, 255, 0);
}
...
void Find_couple_red::find_Contours(Mat & roi){
    ...
    for(int k = 0; k < (int)contour_light.size(); ++k){
        //有父,子轮廓，不满住6点拟合椭圆的先排除
        if(hierarchy[k][3]>0 || hierarchy[k][2]>0 || contour_light[k].size() < 6)
            continue;

        int area = contourArea(contour_light[k]);
        //double length = arcLength(contour_light[k], true);
        //轮廓面积过小或者过大，排除
        if(area < 80 || area > 1e4)
            continue;

        RotatedRect fit_ellipse = fitEllipse(contour_light[k]);
...
double Find_couple_red::is_Horizon(const Point2f & center_i, const Point2f & center_j){
    double gradient = fabs((center_i.y - center_j.y)/(center_i.x - center_j.x));
    //cout << "gradient:" << gradient << endl;
    return gradient;
}

double Find_couple_red::is_Aline(const RotatedRect & rect_led_i, const RotatedRect & rect_led_j){
    double dif_angle;
...
double Find_couple_red::is_Dist_led(const Point2f & point_i, const Point2f & point_j){
...
RotatedRect Find_couple_red::boundRect(const RotatedRect & L_led, const RotatedRect & R_led, const double & W, const double & H){
...
Rect Find_couple_red::get_Roi(const bool & is_target, const Mat & img, const RotatedRect & l, const RotatedRect & r, const double & dis, const double & aver, Mat & src){
    if(is_target){
        lost_cnt = 0;
        //识别坐标并转换
...
void Find_couple_red::make_RotatedSafe(const Mat & bound_size){
    Point2f vertex[4];
